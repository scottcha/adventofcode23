...
fn main() -> io::Result<()>{
    //let path = Path::new("day3_part1_test_input.txt");
    let path = Path::new("day3_part1_input.txt");
    ...
    for row in 0..grid_rows
    {
        ...
        for col in 0..grid_cols
        {
            ...
            else //c == '.'
            {
                if on_part && is_valid_part
                {
                    ...
                    part_number_sum += tmp_part_number.parse::<i32>().unwrap();
        ...
        if on_part && is_valid_part
        {
            //convert part_number to a string
            let tmp_part_number: String = part_number.clone().unwrap().into_iter().collect();
            println!("found valid part_number at end of row: {} ending at row {} col {}", tmp_part_number, row, -1);
            part_number_sum += tmp_part_number.parse::<i32>().unwrap(); 
        }
    }
